  ...
  * of the plot, describing what variables are plotted and how the Bondarenko 
  * differential equation is calculated.
  */
    define(["utility"],
    function BondarenkoForm(utils) {
      "use strict";
      var mediator;
      /**
      ...
      */
      function initialize(defaultSettings, mediatorObj){
        ...
        */
        ...
        var   formCtrls    =  [ "displayAPDDI", "displayS1S2" , "displayV","displayCai","displayCass",                   
                                "displayCajsr","displayCansr","displayLtrpn","displayHtrpn",
                                "displayNai","displayKi","displayAtof","displayItof",
                                "displayNks","displayAtos","displayItos","displayAur",
                                "displayIur","displayXakss","displayXikss","displayOl",
                                "displayOna","displayOk","displayPryr","displayPo12",
                                "secondaryPlot"],                                
                    
              settingCtrls = ["s1","s2","ns1", "paramType","gna", "gcal", "gktof", "gktos", "gkr", "gks", "knaca", "gk1"];
      ...
      */
      ...
      /**
            ...
            */ 
            ...
            function _paramTypeChanged(ele){
               if(ele){      
                  ele.addEventListener("change", function(e) {
                    var selectedVal = ele.options[ele.selectedIndex].value;
                    if(selectedVal){
                       settings.calculationSettings.initCalculationSettings(settings, selectedVal);
                        
                        settingCtrls.forEach( function(ctrl){
                        var ele1 = document.getElementsByName(ctrl)[0]; 
                        if(ele1.type == "select-one"){ // For the currents drop down
                            for (var i = 0, j = ele1.options.length; i < j ; i++) {
                              var option = ele1.options[i];               
                              if (option.value === settings.calculationSettings[ele1.name]) {              
                                ele1.selectedIndex = i;
                                break;
                              }
                            }
                          }
                          else{                                                 
                        ele1.value = settings.calculationSettings[ele1.name];
                      }
                      });                       
                    }
                  });
