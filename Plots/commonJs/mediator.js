...
function NobleMediator() {
	...
	*/
    ...
    function printPoints(settings){

    var pointNames = [];
    var formSettings = settings.formSettings;
        for(var setting in formSettings) {
            if(formSettings.hasOwnProperty(setting)){
                if((formSettings[setting] === true) ||  (formSettings[setting] === 1) ||  (setting === "secondaryPlot") ){                                
                    if(setting === "secondaryPlot"){
                        pointNames.push(formSettings[setting]);
                    }
                    else{
                        if(setting  && (typeof setting === "string") && setting.includes("display")){
                            setting =  setting.replace("display","").toLowerCase();
                            if(settings.calculationSettings.pointBuffer.points[setting]){
                                pointNames.push(setting);                               
                            }  
                        }
                    }
                }
    	...
    	var points = settings.calculationSettings.pointBuffer.points,
            ...
            fileName = (document.title.trim().length > 0)? document.title.trim() : "download",
            normalPoints;
            if(settings.calculationSettings.pointBuffer.normalPoints){
    	    ...
    	    for(var i = 0; i < counter; i++ ){
    	    	...
    	    	for(var j =0, count = pointNames.length; j < count; j++ ){	
    	    		if(normalPoints && normalPoints[pointNames[j]])
    	    		{
